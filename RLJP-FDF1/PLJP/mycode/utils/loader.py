...
def truncate_text(text, max_len=1024):
    if text == "":
        return ""
    text = text.replace("。", "。 ").replace("，", "， ").replace("；", "； ").replace("、", "、 ") # 标点符号后加空格
    text = text.split() # 以空格为分隔符，包含 \n
    if "经审理" in text[0]: # 删除“经审理”这段
        text = text[1:]
    prefix = [] # 前缀
    postfix = [] # 后缀
    n = len(text)
    ...
    while i < n: # 正序
        sent = text[i] 
        prefix = prefix + [sent]
        if len(" ".join(prefix)) >= max_len // 2: #前缀一半长度
            break
        i += 1
    while j > i:  # 倒序
        sent = text[j]
        postfix = [sent] + postfix # 倒序
        if len(" ".join(postfix)) >= max_len // 2: # 后缀一半长度
            break
        j -= 1
    ret_text = prefix + postfix
    return "".join(ret_text) # 为啥最后是无空格连接，因为按照空格分割 空格前是标点符号


def truncate_text_total(text, max_len):
    text = truncate_text(text, max_len, delimiter="。") # 没有这个函数，没有delimiter参数
