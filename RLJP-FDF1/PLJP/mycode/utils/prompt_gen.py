...
'''
get_pt_cls
输入为 数字（月），根据范围划分到pt_cls2str列表里的类别
输出 数字序号，对应到pt_cls2str
'''
def get_pt_cls(pt):
    if pt > 10 * 12:
        pt_cls = 9
...
def label_prompt_case(case, task):
    ...
    if task == "article":
        label = max(case["meta"]["relevant_articles"]) # 只选最大的那条？啊？
        label = f"第{label}条"
    if task == "penalty":
        pt = case["meta"]["term_of_imprisonment"]["imprisonment"]
        pt_cls = get_pt_cls(pt)
        label = pt_cls2str[pt_cls] + "有期徒刑" #其他 怎么就是 有期徒刑了，无期or死刑怎么算？
    return label
...
def retrieved_label_option_fewshot(fact, in_context_content, args,caseID):
    predicted_articles = {} # 预测的法条 【完成后面两个内容的时候需要用】
    ...
    predicted_charges = {} # 预测判决 【完成最后一个内容的时候需要用】
    ...
    predicted_article = predicted_articles[caseID] # 测试案例 预测的法条
    predicted_article_content = loader.truncate_text(loader.load_predicted_article_content(predicted_article), max_len=100) # 测试案例 预测的法条内容
    predicted_charge = predicted_charges[caseID] # 目标 预测的判决
    ...
    examples = in_context_content["precedents"] # 先例列表
    # 0shot
    # examples = []
    label_option = in_context_content["topk_label_option"]

    texts = []
    for case in examples: # 先例列表
        cur_fact = case["fact"].replace(" ", "")
        if args.use_split_fact: # reframed fact 重构事件描述文本
            cur_fact = [case["fact_split"]["zhuguan"], case["fact_split"]["keguan"], case["fact_split"]["shiwai"]] # 提取[主观动机，客观行为，事后情况]三个要素
            cur_fact = "。".join(cur_fact) # 用“。”做分隔符吗？
        cur_label = label_prompt_case(case, args.task) # 找出先例的标签

        max_examplar_len = 256 # max_examplar_len 案例[主观动机，客观行为，事后情况]文本最大长度
        cur_fact = loader.truncate_text(cur_fact, max_len = max_examplar_len) # 截断文本

        text = f"{cur_fact}\n{task_prompt[args.task]}：{cur_label}" # 先例案例+任务词句+标签
        texts.append(text)

    # args.shot含义不确定？猜测：few shot样本的个数
    # label_option 什么含义？ 所有的标签选shot个？不能吧
    ...
    if args.task == "article": # 加入article definition
        ...
        article_definition = [loader.truncate_text(text, max_len=50) for text in article_definition] #截取50个字符够表述清楚含义吗？
        article_definition = article_definition[:args.shot]
        texts += article_definition  # 在[先例案例+任务词句+标签]列表之后加上法条定义？ 不一一对应吗
    
    # texts是先例列表 + shot个article_definition 列表
    examples = "\n\n".join(texts) # 使用换行添加每个先例之间
    examples = examples + "\n\n"
